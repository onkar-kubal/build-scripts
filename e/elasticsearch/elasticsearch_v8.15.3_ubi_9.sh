...
WORKDIR=`pwd`
ELASTICSEARCH_VERSION=${1:-v8.15.3}

# install dependencies
yum install -y wget git zip unzip sudo libtool-ltdl nano
...
# apply patch
# wget https://raw.githubusercontent.com/ppc64le/build-scripts/master/e/elasticsearch/elasticsearch_v8.15.3.patch
git apply elasticsearch_v8.15.3.patch
...
mkdir -p distribution/archives/docker/ubi-docker-ppc64le-export
mkdir -p distribution/archives/docker/cloud-docker-ppc64le-export
mkdir -p distribution/archives/docker/ironbank-docker-ppc64le-export
mkdir -p distribution/archives/docker/cloud-ess-docker-ppc64le-export
